...
public class PathfindTest : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        pathfinding = new Pathfinding(18, 10, new GameObject[] {greenBox,redBox});
    ...
    private void Update()
    {
        ...
        {
            Vector2 trueOrigin = new Vector2(Convert.ToInt32(transform.position.x), Convert.ToInt32(transform.position.y));
            Vector3 mouseWorldPosition = camera.ScreenToWorldPoint(Input.mousePosition);
            mouseWorldPosition += new Vector3(0.5f, 0.5f, 0);
            pathfinding.GetGrid().GetXY(mouseWorldPosition, out int x, out int y);
            path = pathfinding.FindPath(Convert.ToInt32(transform.position.x), Convert.ToInt32(transform.position.y), x, y);
            if (path != null)
            {
