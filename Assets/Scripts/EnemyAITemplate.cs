...
public class EnemyAITemplate : MonoBehaviour
{
    ...
    Vector3 curPos;
    Vector2 targetPos;
    float cooldown; //hyökkäys cooldown
    float endlag; //määrää kuinka kauan vihu paikoillaan attack jälkeen
    ...
    void Update()
    {
        ...
        if (endlag <= 0)
        {
            bestSoFar = 9999;
            autoTarget = null;
            foreach (GameObject target in targetLists.playerTargets) //targettaa lähinpää
            {
